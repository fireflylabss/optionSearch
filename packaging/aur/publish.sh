#!/usr/bin/env bash
# Publish / update optionsearch on the AUR (local helper).
# Usage: ./packaging/aur/publish.sh           # push current packaging/
#        ./packaging/aur/publish.sh 0.1.0     # bump + push
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
AUR_DIR="${AUR_DIR:-$HOME/aur/optionsearch}"
SSH_KEY="${AUR_SSH_KEY:-$HOME/.ssh/aur_synara}"
REMOTE="ssh://aur@aur.archlinux.org/optionsearch.git"
...
if ! git -C "$AUR_DIR" diff --cached --quiet; then
  ...
  git -C "$AUR_DIR" commit -m "optionsearch ${VER}"
fi
